...
def evaluate_celeba_spoof_dataset(model):
    ...
    X_test = LivenessModel.preprocess(X_test)
...
def evaluate_celeba_spoof_dataset_deepface():
    ...
    for img in tqdm(X_test):
        faces = DeepFace.extract_faces(img, detector_backend="yolov8", enforce_detection=False, anti_spoofing=True)
...
def main():
    print("Evaluating Liveness Model")
