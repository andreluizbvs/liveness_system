...
def digital_augment(src_face, mask_path, mode="minor"):
    src_img, tar_img, spatial_transform = st_generator(src_face, mode)

    # mask deformation
    hull_mask = spatial_transform(image=mask_path)["image"]

    elt_mask = elasticdeform.deform_random_grid(hull_mask, sigma=4, points=6)
    ...
    r = (
        random.sample([0.25, 0.5, 0.75, 1, 1, 1], 1)
        if mode == "minor"
        else random.sample([0.75, 1, 1, 1], 1)
    )
    forge_mask = r * blured
...
def st_generator(src_img, mode):
    ...
    color_transform, spatial_transform, _ = transform_params(
        src_img, mode
    )

    select_prob = random.random()
    flag = (
        0 if (select_prob < 0.5) else 1
    )  # color change selection : "0-sr 1-tar"
    if flag == 0:
...
def random_deform(image_size, nrows, ncols, mean=0, std=5):
    """
...
def piecewise_affine_transform(image, src_anchor, tgt_anchor):
    trans = PiecewiseAffineTransform()
    trans.estimate(src_anchor, tgt_anchor)
